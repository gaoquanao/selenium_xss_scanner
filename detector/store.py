#-*- coding:utf-8 -*-
# 存储型XSS漏洞检测，脚本内容为请求一个url，带上id，url根据是否被请求和id来在数据库中设置vul字段
import json
import time
import sqlite3
import urlparse
...
class Detector(object):
    ...
    def init_proxy(self, port):
        ...
        self.proxy.blacklist([r"https?://.+\.(ico|bin|gif|png|webp|jpg|mp4|js|html)"], 404)
    ...
    def queryOne(self):
        ...
        result = cur.execute("SELECT url, headers, method FROM data WHERE id=(?)", (self.url_id,))
        row =  result.fetchone()
        self.url = row[0]
    ...
    def get_query_key(self):
        try:
            return urlparse.parse_qs(urlparse.urlparse(self.url).query).keys()
        except KeyError:
